...
import type { Tomador, Contato } from '../../types';
import { useToast } from '../../contexts/ToastContext';

interface TomadorFormProps {
    onSuccess: () => void;
    onCancel: () => void;
    initialData?: Partial<Tomador>;
}
...
export const TomadorForm: React.FC<TomadorFormProps> = ({ onSuccess, onCancel, initialData }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (initialData?.id) {
                await api.put(`/tomadores/${initialData.id}`, payload);
            } else {
                await api.post('/tomadores', payload);
            }
            showToast(`Tomador ${initialData?.id ? 'atualizado' : 'criado'} com sucesso!`, 'success');
            onSuccess();
        } catch (error) {
            console.error('Erro ao salvar tomador', error);
            showToast('Erro ao salvar tomador. Tente novamente.', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto px-1">
            ...
            <div className="flex justify-end gap-3">
                ...
                <button type="submit" disabled={loading}
                    ...
                    {loading ? 'Salvando...' : 'Salvar Tomador'}
                </button>
            </div>
        </form>
    );
